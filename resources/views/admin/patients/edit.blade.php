...
<div>
             ...
             @endif --}}
              <form action="{{ route('admin.patients.update',[$department->id,$patient->id]) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <div class="card-body">
                  <div class="form-group">
                    <label for="phone_num">Phone Number</label>
                    <input type="text" class="form-control @error('phone_num') is-invalid @enderror" id="phone_num" name="phone_num" value="{{ old ('phone_num',$patient->phone_num) }}" >
                     <x-errors name="phone_num"/>
                  </div>
                   ...
                   <div class="form-group">
                    ...
                    <select name="country" class="form-control @error('country') is-invalid @enderror select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="8" tabindex="-1" aria-hidden="true">
                     ...
                     @foreach (\App\Enum\CountryNameStatus::cases() as $country)
                         <option value="{{ $country->value }}"  {{ old('country', $patient->country ?? '') == $country->value ? 'selected' : '' }}>{{ $country->label() }}</option>
                     @endforeach
                    </select>
