 ...
 @section('title',' الصفحة الرئيسية لعروض السعر')
 @push('css')
            ...
            <div class="row">
                <div class="col-sm-12">
                  ...
                  <tr role="row">
                    ...
                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Suggested Department</th>
                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">Services Requested</th>
                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">Last Test</th>
                                        <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">Order Status</th>
                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">Hospital Name</th>
                  ...
                  @foreach ($priceQuotes as $pricequote)
                      ...
                      <td>{{ $pricequote->suggested_department_id }}</td>
                      <td>{{ $pricequote->services_requested->label() }}</td>
                      <td>{{ $pricequote->last_test->label() }}</td>
                      <td>{{ $pricequote->order_status->label() }}</td>
                      <td>{{ $hospital->name}}</td>
                      
                      <td>
                        <a class="btn btn-primary" href="{{ route('admin.priceQuotes.show',[$hospital->id,$pricequote->id]) }}"><i class="fa-solid fa-eye"></i></a>
                        <a class="btn btn-warning" href="{{ route('admin.priceQuotes.edit',[$hospital->id,$pricequote->id]) }}"><i class="fa-solid fa-pen-to-square"></i></a>
                      <form action="{{ route('admin.priceQuotes.delete',[$hospital->id,$pricequote->id]) }}" 
                            method="POST" 
